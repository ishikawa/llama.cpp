...
kernel void kernel_mul_mm_id_map0(
    ...
    // A row of ids can reference the same expert multiple times (for example,
    // hash-routing tables remapped after expert pruning), so each expert needs
    // capacity for up to ne20*ne21 entries.
    ...
    for (int i21 = 0; i21 < args.ne21; i21 += ntg) { // n_tokens
        ...
        for (short t = 0; t < ntg; t++) {
            ...
            // Emit one entry per matching slot. Duplicate ids within a row must
            // each produce their own output row.
            #pragma unroll(ne20)
            for (short i20 = 0; i20 < ne20; i20++) {
                if (sids[i20] == ide) {
...
kernel void kernel_mul_mm_id(
    ...
    // Per-expert capacity is ne20*ne21 entries because rows can reference the
    // same expert in multiple slots.
