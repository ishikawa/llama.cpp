...
template [[host_name("kernel_flash_attn_ext_vec_f16_dk512_dv512_q3_ne1")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 512, 512, 1, 3>;
template [[host_name("kernel_flash_attn_ext_vec_f16_dk512_dv512_q3_ne2")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 512, 512, 2, 3>;
template [[host_name("kernel_flash_attn_ext_vec_f16_dk512_dv512_q3_ne4")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, half4, 1, dequantize_f16_t4, 512, 512, 4, 3>;
#if defined(GGML_METAL_HAS_BF16)
...
constant int32_t FC_flash_attn_ext_vec_mla_NWG [[function_constant(FC_FLASH_ATTN_EXT_VEC_MLA + 0)]];

kernel void kernel_flash_attn_ext_vec_mla_f16_dk512_dv512(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    const int qr = qrow0 + sgitg;
    const bool valid_q = qr < nqr;
    const int q1 = valid_q ? qr%args.ne01 : 0;
    const int q2 = valid_q ? qr/args.ne01 : 0;
    const int ikv2 = valid_q ? q2/(args.ne02/args.ne_12_2) : 0;
    ...
    device const half * pm = valid_q ? (device const half *) (mask + q1*args.nb31 + (q2%args.ne32)*args.nb32 + (q3%args.ne33)*args.nb33) : (device const half *) mask;
    ...
    for (int ic = iwg*K_TILE; ic < args.ne11; ic += NWG*K_TILE) {
        ...
        const float M_new = max(M, mt);
        const float alpha = M <= -FLT_MAX/4 ? 0.0f : exp(M - M_new);

        float probs[K_TILE];
        float psum = 0.0f;

        for (short kc = 0; kc < K_TILE; ++kc) {
            probs[kc] = scores[kc] <= -FLT_MAX/4 ? 0.0f : exp(scores[kc] - M_new);
            psum += probs[kc];
        }
    ...
    if (valid_q) {
        ...
        device float4 * htmp4 = (device float4 *) htmp;
        device float  * ss    = (device float  *) htmp + (uint64_t)nrows*DV*NWG;

        for (short i = 0; i < NO; ++i) {
            const short id = tiisg + i*NW;
            htmp4[(uint64_t)rid*DV4*NWG + NWG*id + iwg] = acc[i];
        }

        if (tiisg == 0) {
            ss[rid*(2*NWG) + 2*iwg + 0] = S;
            ss[rid*(2*NWG) + 2*iwg + 1] = M;
        }
    }
...
kernel void kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v3a(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    constexpr short M_SUB = 8;
    constexpr short ROWS_PER_SG = 4;
    constexpr short K_SUB = 8;
    ...
    threadgroup half  * sq  = shmem_f16;
    threadgroup half4 * sq4 = (threadgroup half4 *) sq;
    threadgroup float * ss  = (threadgroup float *) (shmem_f16 + M_CHUNK*DK);
    ...
    for (int i = tiitg; i < M_CHUNK*DK4; i += OP_FLASH_ATTN_EXT_VEC_MLA_Q_TILE*NW) {
        const int iq = i/DK4;
        const int id = i - iq*DK4;
        const int qr = qrow0 + iq;
    ...
    // Each simdgroup owns four M rows: sgitg, sgitg+4, sgitg+8, sgitg+12.
    float4 acc[ROWS_PER_SG][NO];
    float S[ROWS_PER_SG];
    float M[ROWS_PER_SG];

    for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
        for (short i = 0; i < NO; ++i) {
            acc[ir][i] = 0.0f;
        }
        S[ir] = 0.0f;
    ...
    for (int ic = iwg*K_TILE; ic < args.ne11; ic += NWG*K_TILE) {
        const short kc0 = sgitg*K_SUB;
        const bool full_ksub = ic + kc0 + K_SUB <= args.ne11;

        if (full_ksub) {
            device const half * pk = (device const half *) (k_base + (ic + kc0)*args.nb11);
            ...
            for (short ii = 0; ii < DK/16; ++ii) {
                ...
                simdgroup_load(mq[0], sq + M_SUB*DK + 16*ii,     DK);
                simdgroup_load(mq[1], sq + M_SUB*DK + 16*ii + 8, DK);
            ...
            simdgroup_store(mqk0, ss + kc0,             K_TILE, 0, false);
            simdgroup_store(mqk1, ss + M_SUB*K_TILE + kc0, K_TILE, 0, false);
        } else {
            for (int i = tiisg; i < M_CHUNK*K_SUB; i += NW) {
                const int iq = i/K_SUB;
                const int kc = kc0 + i - iq*K_SUB;
                const int qr = qrow0 + iq;
                const int kv = ic + kc;

                float qk = 0.0f;
                if (qr < nqr && kv < args.ne11) {
                    device const half4 * pk4 = (device const half4 *) (k_base + kv*args.nb11);
                    for (short id = 0; id < DK4; ++id) {
                        qk += dot((float4) sq4[iq*DK4 + id], (float4) pk4[id]);
                    }
                }
        ...
        for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
            const int iq = sgitg + ir*OP_FLASH_ATTN_EXT_VEC_MLA_Q_TILE;
            const int qr = qrow0 + iq;
            const bool valid_q = qr < nqr;
            const int q1 = valid_q ? qr%args.ne01 : 0;
            const int q2 = valid_q ? qr/args.ne01 : 0;
            ...
            for (short kc = 0; kc < K_TILE; ++kc) {
                const bool valid_k = ic + kc < args.ne11;
                const float mv = valid_q && valid_k ? (float) pm[ic + kc] : -MAXHALF;
                scores[kc] = valid_q && valid_k && mv > -MAXHALF ? fma(ss[iq*K_TILE + kc], args.scale, mv) : -FLT_MAX/2;
            }
            ...
            const float M_new = max(M[ir], mt);
            const float alpha = M[ir] <= -FLT_MAX/4 ? 0.0f : exp(M[ir] - M_new);
    ...
    for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
        ...
        if (qr < nqr) {
            ...
            for (short i = 0; i < NO; ++i) {
                const short id = tiisg + i*NW;
                htmp4[(uint64_t)rid*DV4*NWG + NWG*id + iwg] = acc[ir][i];
            }

            if (tiisg == 0) {
                dsts[rid*(2*NWG) + 2*iwg + 0] = S[ir];
                dsts[rid*(2*NWG) + 2*iwg + 1] = M[ir];
            }
        }
    }
...
static ushort2 flash_attn_mla_morton_order(ushort lane) {
    const ushort qid = lane/4;
    const ushort row = (qid & 4) + (lane/2)%4;
    const ushort col = (qid & 2)*2 + (lane%2)*2;
    return ushort2(col, row);
}

kernel void kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v4b(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    constexpr short DK4 = DK/4;
    constexpr short M_CHUNK = 16;
    constexpr short M_PAD = 20;
    constexpr short ROWS_PER_SG = 4;
    constexpr short K_TILE = 8;
    constexpr short NW = N_SIMDWIDTH;
    constexpr short NO = DV/K_TILE;
    ...
    for (int i = tiitg; i < M_PAD*DK4; i += OP_FLASH_ATTN_EXT_VEC_MLA_Q_TILE*NW) {
        const int iq = i/DK4;
        const int id = i - iq*DK4;
        const int qr = qrow0 + iq;

        if (iq < M_CHUNK && qr < nqr) {
            const int q1 = qr%args.ne01;
            const int q2 = qr/args.ne01;
            device const float4 * pq4 = (device const float4 *) (q + q1*args.nb01 + q2*args.nb02 + q3*args.nb03);
            sq4[i] = (half4) pq4[id];
    ...
    const ushort2 tc = flash_attn_mla_morton_order(tiisg);
    const short local_col0 = tc.x;
    const short local_col1 = local_col0 + 1;
    const short local_row  = tc.y;
    const bool real_row = local_row < ROWS_PER_SG;

    const int iq = sgitg*ROWS_PER_SG + local_row;
    const int qr = qrow0 + iq;
    const bool valid_q = real_row && qr < nqr;
    const int q1 = valid_q ? qr%args.ne01 : 0;
    const int q2 = valid_q ? qr/args.ne01 : 0;
    const int ikv3 = q3/(args.ne03/args.ne_12_3);
    ...
    for (int ic = iwg*K_TILE; ic < args.ne11; ic += NWG*K_TILE) {
        const bool full_tile = ic + K_TILE <= args.ne11;
        ...
        if (full_tile) {
            ...
            for (short ii = 0; ii < DK/16; ++ii) {
                ...
                simdgroup_load(mq[0], sq + (sgitg*ROWS_PER_SG)*DK + 16*ii,     DK);
                simdgroup_load(mq[1], sq + (sgitg*ROWS_PER_SG)*DK + 16*ii + 8, DK);
            ...
            float score0 = mqk.thread_elements()[0];
            float score1 = mqk.thread_elements()[1];

            if (valid_q) {
                const float mv0 = (float) pm[ic + local_col0];
                const float mv1 = (float) pm[ic + local_col1];
                score0 = mv0 > -MAXHALF ? fma(score0, args.scale, mv0) : -FLT_MAX/2;
                score1 = mv1 > -MAXHALF ? fma(score1, args.scale, mv1) : -FLT_MAX/2;
            } else {
                score0 = -FLT_MAX/2;
                score1 = -FLT_MAX/2;
            }

            float mt = max(score0, score1);
            mt = max(mt, simd_shuffle_xor(mt, 1));
            mt = max(mt, simd_shuffle_xor(mt, 8));

            const float M_new = max(M, mt);
            const float alpha = M <= -FLT_MAX/4 ? 0.0f : exp(M - M_new);

            const float p0 = score0 <= -FLT_MAX/4 ? 0.0f : exp(score0 - M_new);
            const float p1 = score1 <= -FLT_MAX/4 ? 0.0f : exp(score1 - M_new);

            float psum = p0 + p1;
            psum += simd_shuffle_xor(psum, 1);
            psum += simd_shuffle_xor(psum, 8);
            ...
            mp.thread_elements()[0] = (half) p0;
            mp.thread_elements()[1] = (half) p1;

            for (short dv0 = 0; dv0 < DV; dv0 += K_TILE) {
                device const half * pv = (device const half *) (v_base + ic*args.nb21);

                simdgroup_float8x8 mo = make_filled_simdgroup_matrix<float, 8>(0.0f);
                simdgroup_half8x8 mv;

                simdgroup_barrier(mem_flags::mem_none);
                simdgroup_load(mv, pv + dv0, DV, 0, false);
                simdgroup_barrier(mem_flags::mem_none);

                simdgroup_multiply_accumulate(mo, mp, mv, mo);

                acc[dv0/K_TILE] += float2(mo.thread_elements()[0], mo.thread_elements()[1]);
            }
        } else if (valid_q) {
            for (short kk = 0; kk < K_TILE && ic + kk < args.ne11; ++kk) {
                float qk = 0.0f;
                device const half4 * pk4 = (device const half4 *) (k_base + (ic + kk)*args.nb11);
                for (short id = 0; id < DK4; ++id) {
                    qk += dot((float4) sq4[iq*DK4 + id], (float4) pk4[id]);
                }

                const float mv = (float) pm[ic + kk];
                const float score = mv > -MAXHALF ? fma(qk, args.scale, mv) : -FLT_MAX/2;
                const float M_new = max(M, score);
                const float alpha = M <= -FLT_MAX/4 ? 0.0f : exp(M - M_new);
                const float p = score <= -FLT_MAX/4 ? 0.0f : exp(score - M_new);
                ...
                device const half * pv = (device const half *) (v_base + (ic + kk)*args.nb21);
                for (short dv0 = 0; dv0 < DV; dv0 += K_TILE) {
                    acc[dv0/K_TILE] += float2((float) pv[dv0 + local_col0], (float) pv[dv0 + local_col1])*p;
                }
            }
        }
    }
    ...
    if (valid_q) {
        ...
        for (short dv0 = 0; dv0 < DV; dv0 += K_TILE) {
            const short dv1 = dv0 + local_col0;
            const short dv2 = dv0 + local_col1;
            htmpf[((uint64_t)rid*(DV/4)*NWG + (dv1/4)*NWG + iwg)*4 + (dv1 & 3)] = acc[dv0/K_TILE][0];
            htmpf[((uint64_t)rid*(DV/4)*NWG + (dv2/4)*NWG + iwg)*4 + (dv2 & 3)] = acc[dv0/K_TILE][1];
        }

        if (local_col0 == 0) {
            dsts[rid*(2*NWG) + 2*iwg + 0] = S;
            dsts[rid*(2*NWG) + 2*iwg + 1] = M;
        }
    }
...
kernel void kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v4c(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    constexpr short N_SUB = N_TILE/K_SUB;
    constexpr short NW = N_SIMDWIDTH;
    constexpr short NO = DV/K_SUB;
    ...
    for (int ic = iwg*N_TILE; ic < args.ne11; ic += NWG*N_TILE) {
        const bool full_tile = ic + N_TILE <= args.ne11;

        if (full_tile) {
            device const half * pk = (device const half *) (k_base + ic*args.nb11);

            simdgroup_float8x8 mqk[N_SUB];
            simdgroup_half8x8 mq[2];
            simdgroup_half8x8 mk[2];

            for (short ks = 0; ks < N_SUB; ++ks) {
                mqk[ks] = make_filled_simdgroup_matrix<float, 8>(0.0f);
            }
            ...
            for (short ii = 0; ii < DK/16; ++ii) {
                ...
                for (short ks = 0; ks < N_SUB; ++ks) {
                    ...
                    simdgroup_load(mk[0], pk + ks*K_SUB*DK + 16*ii,     DK, 0, true);
                    simdgroup_load(mk[1], pk + ks*K_SUB*DK + 16*ii + 8, DK, 0, true);

                    simdgroup_barrier(mem_flags::mem_none);

                    simdgroup_multiply_accumulate(mqk[ks], mq[0], mk[0], mqk[ks]);
                    simdgroup_multiply_accumulate(mqk[ks], mq[1], mk[1], mqk[ks]);
                }
            }
            ...
            for (short ks = 0; ks < N_SUB; ++ks) {
                float s0 = mqk[ks].thread_elements()[0];
                float s1 = mqk[ks].thread_elements()[1];

                if (valid_q) {
                    const int kc0 = ic + ks*K_SUB + local_col0;
                    const int kc1 = ic + ks*K_SUB + local_col1;
                    const float mv0 = (float) pm[kc0];
                    const float mv1 = (float) pm[kc1];
                    s0 = mv0 > -MAXHALF ? fma(s0, args.scale, mv0) : -FLT_MAX/2;
                    s1 = mv1 > -MAXHALF ? fma(s1, args.scale, mv1) : -FLT_MAX/2;
                } else {
                    s0 = -FLT_MAX/2;
                    s1 = -FLT_MAX/2;
                }

                score[ks] = float2(s0, s1);
                mt = max(mt, max(s0, s1));
            }
            ...
            for (short ks = 0; ks < N_SUB; ++ks) {
                const float p0 = score[ks][0] <= -FLT_MAX/4 ? 0.0f : exp(score[ks][0] - M_new);
                const float p1 = score[ks][1] <= -FLT_MAX/4 ? 0.0f : exp(score[ks][1] - M_new);
                prob[ks] = float2(p0, p1);
                psum += p0 + p1;
            }
            ...
            for (short dv0 = 0; dv0 < DV; dv0 += K_SUB) {
                simdgroup_float8x8 mo = make_filled_simdgroup_matrix<float, 8>(0.0f);

                for (short ks = 0; ks < N_SUB; ++ks) {
                    device const half * pv = (device const half *) (v_base + (ic + ks*K_SUB)*args.nb21);

                    simdgroup_half8x8 mp;
                    simdgroup_half8x8 mv;

                    mp.thread_elements()[0] = (half) prob[ks][0];
                    mp.thread_elements()[1] = (half) prob[ks][1];
                ...
                acc[dv0/K_SUB] += float2(mo.thread_elements()[0], mo.thread_elements()[1]);
            }
        } else if (valid_q) {
            for (short kk = 0; kk < N_TILE && ic + kk < args.ne11; ++kk) {
                float qk = 0.0f;
                device const half4 * pk4 = (device const half4 *) (k_base + (ic + kk)*args.nb11);
                for (short id = 0; id < DK4; ++id) {
                    qk += dot((float4) sq4[iq*DK4 + id], (float4) pk4[id]);
                ...
                device const half * pv = (device const half *) (v_base + (ic + kk)*args.nb21);
                for (short dv0 = 0; dv0 < DV; dv0 += K_SUB) {
                    acc[dv0/K_SUB] += float2((float) pv[dv0 + local_col0], (float) pv[dv0 + local_col1])*p;
                }
            }
        }
    }
    ...
    if (valid_q) {
        ...
        for (short dv0 = 0; dv0 < DV; dv0 += K_SUB) {
            const short dv1 = dv0 + local_col0;
            const short dv2 = dv0 + local_col1;
            htmpf[((uint64_t)rid*(DV/4)*NWG + (dv1/4)*NWG + iwg)*4 + (dv1 & 3)] = acc[dv0/K_SUB][0];
            htmpf[((uint64_t)rid*(DV/4)*NWG + (dv2/4)*NWG + iwg)*4 + (dv2 & 3)] = acc[dv0/K_SUB][1];
        }
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    threadgroup half4 * sq4 = (threadgroup half4 *) sq;
    threadgroup float * ss  = (threadgroup float *) (shmem_f16 + M_CHUNK*DK);
    threadgroup half  * sp  = (threadgroup half *) (ss + M_CHUNK*K_TILE);
    threadgroup float * so  = (threadgroup float *) (sp + OP_FLASH_ATTN_EXT_VEC_MLA_Q_TILE*M_SUB*K_TILE);

    threadgroup half  * sp_sg = sp + sgitg*M_SUB*K_TILE;
    threadgroup float * so_sg = so + sgitg*M_SUB*M_SUB;
    ...
    // PV uses four owned rows plus four zero rows, so this stage only changes PV.
    float4 acc[ROWS_PER_SG][NO];
    float S[ROWS_PER_SG];
    float M[ROWS_PER_SG];
    ...
    for (int ic = iwg*K_TILE; ic < args.ne11; ic += NWG*K_TILE) {
        ...
        for (int i = tiisg; i < M_SUB*K_TILE; i += NW) {
            sp_sg[i] = 0.0h;
        }
        ...
        for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
            ...
            for (short kc = 0; kc < K_TILE; ++kc) {
                const float p = scores[kc] <= -FLT_MAX/4 ? 0.0f : exp(scores[kc] - M_new);
                psum += p;
                sp_sg[ir*K_TILE + kc] = (half) p;
            }
        ...
        for (short dv0 = 0; dv0 < DV; dv0 += M_SUB) {
            simdgroup_float8x8 mpv = make_filled_simdgroup_matrix<float, 8>(0.0f);

            for (short ks = 0; ks < K_TILE/K_SUB; ++ks) {
                const int kv0 = ic + ks*K_SUB;
                const bool full_vsub = kv0 + K_SUB <= args.ne11;

                if (full_vsub) {
                    device const half * pv = (device const half *) (v_base + kv0*args.nb21);
                    simdgroup_half8x8 mp;
                    simdgroup_half8x8 mv;

                    simdgroup_barrier(mem_flags::mem_none);
                    simdgroup_load(mp, sp_sg + ks*K_SUB, K_TILE, 0, false);
                    simdgroup_load(mv, pv + dv0, DV, 0, false);
                    simdgroup_barrier(mem_flags::mem_none);

                    simdgroup_multiply_accumulate(mpv, mp, mv, mpv);
                }
            }

            simdgroup_store(mpv, so_sg, M_SUB, 0, false);
            ...
            for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
                for (short i = 0; i < NO; ++i) {
                    const short id = tiisg + i*NW;
                    const short col4 = 4*id;

                    if (col4 >= dv0 && col4 < dv0 + M_SUB) {
                        const short off = col4 - dv0;
                        acc[ir][i] += float4(
                            so_sg[ir*M_SUB + off + 0],
                            so_sg[ir*M_SUB + off + 1],
                            so_sg[ir*M_SUB + off + 2],
                            so_sg[ir*M_SUB + off + 3]);
                    }
                }
            }
        ...
        for (short ks = 0; ks < K_TILE/K_SUB; ++ks) {
            ...
            if (!full_vsub) {
                for (short kk = 0; kk < K_SUB; ++kk) {
                    if (kv0 + kk >= args.ne11) {
                        break;
                    }

                    device const half4 * pv4 = (device const half4 *) (v_base + (kv0 + kk)*args.nb21);
                    ...
                    for (short ir = 0; ir < ROWS_PER_SG; ++ir) {
                        ...
                        if (qr < nqr) {
                            const float p = (float) sp_sg[ir*K_TILE + ks*K_SUB + kk];
...
template<short NQ>
kernel void kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6(
        constant ggml_metal_kargs_flash_attn_ext_vec & args,
        device const char * q,
        device const char * k,
        device const char * v,
...
#define NWG FC_flash_attn_ext_vec_mla_NWG
    ...
    constexpr short NW = N_SIMDWIDTH;
    constexpr short QI = DK4/NW;
    constexpr short NO = DV4/NW;

    static_assert(NQ == 2 || NQ == 3, "NQ must be 2 or 3");
    static_assert(C == 32, "C must be 32");

    const int qrow0 = tgpig[0]*NQ;
    const int q2    = qrow0/args.ne01;
    const int q3    = tgpig[1];
    const int iwg   = tgpig[2];

    half4 lq[NQ][QI];

    FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
        device const float4 * pq4 = (device const float4 *) (q + iq*args.nb01 + q2*args.nb02 + q3*args.nb03);

        FOR_UNROLL (short ii = 0; ii < QI; ++ii) {
            lq[iq][ii] = (half4) pq4[tiisg + ii*NW];
        }
    }

    const int ikv2 = q2/(args.ne02/args.ne_12_2);
    const int ikv3 = q3/(args.ne03/args.ne_12_3);
    ...
    FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
        pm[iq] = (device const half *) (mask + iq*args.nb31 + (q2%args.ne32)*args.nb32 + (q3%args.ne33)*args.nb33);
    }
    ...
    for (int ic = iwg*C; ic < args.ne11; ic += NWG*C) {
        float score[NQ];
        float prob[NQ];
        ...
        FOR_UNROLL (short cc = 0; cc < C; ++cc) {
            device const half4 * pk4 = (device const half4 *) (k_base + (ic + cc)*args.nb11);
            float qk[NQ];
            ...
            FOR_UNROLL (short ii = 0; ii < QI; ++ii) {
                const half4 mk = pk4[tiisg + ii*NW];

                FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
                    qk[iq] += dot((float4) mk, (float4) lq[iq][ii]);
                }
            }

            FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
                const float sqk = simd_sum(qk[iq]);

                if (cc == tiisg) {
                    const float mv = (float) pm[iq][ic + cc];
                    score[iq] = mv > -MAXHALF ? fma(sqk, args.scale, mv) : -FLT_MAX/2;
                }
            }
        }

        FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
            const float mt = simd_max(score[iq]);
            const float M_new = max(M[iq], mt);
            const float alpha = M[iq] <= -FLT_MAX/4 ? 0.0f : exp(M[iq] - M_new);

            prob[iq] = score[iq] <= -FLT_MAX/4 ? 0.0f : exp(score[iq] - M_new);

            S[iq] = S[iq]*alpha + simd_sum(prob[iq]);
            M[iq] = M_new;
        ...
        FOR_UNROLL (short cc = 0; cc < C; ++cc) {
            device const half4 * pv4 = (device const half4 *) (v_base + (ic + cc)*args.nb21);
            float ps[NQ];

            FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
                ps[iq] = simd_shuffle(prob[iq], cc);
            }

            FOR_UNROLL (short i = 0; i < NO; ++i) {
                const half4 mvh = pv4[tiisg + i*NW];
                const float4 mv = (float4) mvh;
    ...
    FOR_UNROLL (short iq = 0; iq < NQ; ++iq) {
        const int rid = q3*args.ne2*args.ne1 + q2 + iq*args.ne1;

        FOR_UNROLL (short i = 0; i < NO; ++i) {
            const short id = tiisg + i*NW;
            htmp4[(uint64_t)rid*DV4*NWG + NWG*id + iwg] = acc[iq][i];
        }

        if (tiisg == 0) {
            dsts[rid*(2*NWG) + 2*iwg + 0] = S[iq];
            dsts[rid*(2*NWG) + 2*iwg + 1] = M[iq];
        }
    }
...
typedef decltype(kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6<2>) flash_attn_ext_vec_mla_v6_t;

template [[host_name("kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6_nq2")]] kernel flash_attn_ext_vec_mla_v6_t kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6<2>;
template [[host_name("kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6_nq3")]] kernel flash_attn_ext_vec_mla_v6_t kernel_flash_attn_ext_vec_mla_f16_dk512_dv512_v6<3>;
